...
CKeyBoard::CKeyBoard()
{
	...
	//bKeyboard = true;
	if(!pSettings->Get().iAndroidKeyboard) EnableOldKeyboard();
	else EnableNewKeyboard();
}
...
void CKeyBoard::Render()
{
	...
	ImVec2 vecButSize = ImVec2(pGUI->ScaleY(140), pGUI->ScaleY(140));
  	...
  	ImGui::PushStyleColor(ImGuiCol_Button, (ImVec4)ImColor(0.10f, 0.10f, 0.1f, 0.75f).Value);

  	// ---> Setup style --------------------
  	pGUI->SetupKeyboardStyle();

    // ---> Load textures --------------------
    UPTexture      = (RwTexture*)LoadTextureFromDB("txd", "up");
    DownTexture    = (RwTexture*)LoadTextureFromDB("txd", "down");
	// ---> Load textures --------------------
	...
	ImGui::GetOverlayDrawList()->AddLine(
		...
		ImVec2(m_Size.x, m_Pos.y + m_fKeySizeY), ImColor(0xFF3291F5));
			...
			switch(key.type)
			{
				...
				case KEY_SHIFT:
					ImGui::GetOverlayDrawList()->AddTriangleFilled(
						...
						m_iCase == LOWER_CASE ? ImColor(0xFF8A8886) : ImColor(0xFF3291F5));
					ImGui::GetOverlayDrawList()->AddRectFilled(
				...
				case KEY_BACKSPACE:
					...
					ImGui::GetOverlayDrawList()->AddConvexPolyFilled(points, 5, 
						key.id == m_iPushedKey && !m_bHidenInput ? ImColor(0xFF3291F5) : ImColor(0xFF8A8886));
				break;
...
void CKeyBoard::HandleInput(kbKey &key)
{
	switch(key.type)
	{
		...
		case KEY_NUMPAD:
		{
			if(m_iLayout == LAYOUT_NUM) m_iLayout = m_OldiLayout;
			else m_OldiLayout = m_iLayout, m_iLayout = LAYOUT_NUM;
			m_iCase = LOWER_CASE;
			break;
		}
...
kbKey* CKeyBoard::GetKeyFromPos(int x, int y)
{
	...
	for(kbKey key : m_Rows[m_iLayout][iRow])
	{
...
void CKeyBoard::InitENG()
{
	...
	curPos = ImVec2(0, m_Pos.y + 5);
	
	ImGuiIO& io = ImGui::GetIO();
	ImVec2 vecButSize = ImVec2(pGUI->ScaleY(145), pGUI->ScaleY(80));
	ImVec2 PositionButtion = ImVec2(io.DisplaySize.x - (vecButSize.x * 4) - 100, (io.DisplaySize.y / 2 - vecButSize.y * 2) + 110);
	//ImVec2 PositionButtion = ImVec2(curPos.x + defWidth * 0.4, curPos.y);
	//PositionButtion.x += PositionButtion.x * 16;
	key.pos = PositionButtion;
