		    ...
		    for(auto& playerkill : m_pDeathMessage)
		    {
			    if(playerkill)
			    {
				    ...
				    if(!playerkill->killerName.empty() && !playerkill->playerName.empty())
				    {
					    ...
					    pGUI->RenderTextDeathMessage(vecPos, ImColor(PlayerColor[0], PlayerColor[1], PlayerColor[3]), true, playerName);
					    ...
					    pGUI->RenderTextDeathMessage(vecPos, ImColor(KillerColor[0], KillerColor[1], KillerColor[3]), true, killerName);
				    }
				    else if(playerkill->killerName.empty() && !playerkill->playerName.empty())
				    {
					    ...
					    if(playerkill->reason == 200) dwColor = 0xFFFF0000;
					    else if(playerkill->reason == 201) dwColor = 0xFF0000FF;

					    vecPos.x = fPosSprite;
					    pGUI->RenderTextDeathMessage(vecPos, 0xFF000000, false, "G", nullptr, 0.0f, pGUI->GetWeaponFont());
				 	    pGUI->RenderTextDeathMessage(vecPos, dwColor, false, SpriteIDForWeapon(playerkill->reason), nullptr, 0.0f, pGUI->GetWeaponFont());
				    }
